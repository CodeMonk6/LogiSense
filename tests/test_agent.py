...
from logisense.agent import MitigationAgent, MitigationAction, RewardFunction
...
class TestRewardFunction:
    def test_reward_range(self):
        rf   = RewardFunction()
        snap = {f"node_{i}": {"fill_rate": np.random.uniform(0.7, 1.0)} for i in range(5)}
...
class TestMitigationAgent:
    ...
    def test_action_types(self, agent, twin):
        ...
        for a in agent.act(state):
            ...
            assert a.action_type in ("reroute", "reallocate", "procure",
                                     "expedite", "hedge", "noop")
    ...
    def test_save_load(self, agent, tmp_path):
        ...
        loaded = MitigationAgent.from_pretrained(str(tmp_path),
                                                  obs_dim=agent.policy.obs_dim,
