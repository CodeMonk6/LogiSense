...
class DigitalTwin:
    ...
    def inject_disruption(self, node_id: str, status: NodeStatus = NodeStatus.CLOSED) -> None:
    ...
    def step(self) -> TwinState:
        ...
        state = TwinState(
            obs=obs, snapshot=snap, day=self.sim.day,
            risk_7d=dict(self._risk_7d), risk_14d=dict(self._risk_14d),
            kpis=kpis,
        )
    ...
    def reset(self) -> TwinState:
        ...
        self._history.clear()
        snap  = {nid: {"inventory": n.inventory, "status": 0, "fill_rate": 1.0,
                        "demand": 0.0, "stockout_days": 0, "capacity": n.capacity,
                        "risk_score": 0.0}
                 for nid, n in self.network.nodes.items()}
        obs   = self.encoder.encode(snap)
        return TwinState(obs=obs, snapshot=snap, day=0,
                         risk_7d={}, risk_14d={}, kpis={})
    ...
    def _compute_kpis(self, snap: Dict[str, dict]) -> Dict[str, float]:
        ...
        high_risk_cnt = sum(1 for n in self.network.nodes.values() if n.risk_score > 0.6)
