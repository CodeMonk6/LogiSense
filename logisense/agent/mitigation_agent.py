...
from logisense.agent.action_space import NOOP, ActionSpace, ActionSpec
...
class MitigationAction:
    ...
    def __repr__(self) -> str:
        return (
            f"[{self.priority}] {self.description} (cost=${self.estimated_cost:,.0f})"
        )
...
class MitigationAgent:
    ...
    def act(
        ...
        self.policy.eval()
        obs_t = torch.tensor(
            state.obs, dtype=torch.float32, device=self._dev
        ).unsqueeze(0)
        ...
        for idx in top_indices:
            ...
            target = (
                max(risk_scores, key=risk_scores.get) if risk_scores else "node_000"
            )
        ...
        logger.info(
            "[Agent] %d action(s) selected (value=%.3f)",
            len(actions),
            float(values.item()),
        )
        return actions
