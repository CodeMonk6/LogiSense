...
SOURCE_DIMS   = [N_SATELLITE_FEATURES, N_WEATHER_FEATURES, N_GEO_FEATURES, N_SENTIMENT_FEATURES]
...
class NodeSignal:
    ...
    def feature_vector(self) -> np.ndarray:
        return np.concatenate([self.satellite, self.weather,
                               self.geopolitics, self.sentiment])
...
class FusedSignalState:
    ...
    def top_risk_nodes(self, n: int = 10) -> List[Tuple[str, float]]:
        pairs = sorted(zip(self.node_ids, self.risk_scores.tolist()),
...
class CrossSourceAttention(nn.Module):
    ...
    def forward(
        ...
        src_seq = torch.stack([proj(s) for proj, s in
                               zip(self.source_projs, splits)], dim=1)  # (N,4,d)
...
class TemporalEncoder(nn.Module):
    ...
    def __init__(self, d_model: int = 128, n_heads: int = 4,
                 T_max: int = 60, dropout: float = 0.1):
        super().__init__()
...
class SignalFusionEngine(nn.Module):
    ...
    def fetch_and_fuse(
        ...
        hist = hist + 0.05 * torch.randn(N, self.T_lookback, D_SIGNAL,
                                         generator=rng, device=self._dev)
        hist = torch.clamp(hist, 0.0, 1.0)
        hist[:, -1, :] = latest   # last step = actual signal

        # Encode
        nt_tensor    = torch.tensor([node_types.get(nid, 0) for nid in node_ids],
        ...
        for i, nid in enumerate(node_ids):
            ...
            node_signals[nid] = NodeSignal(
                node_id=nid, timestamp=timestamp,
        ...
        logger.info(f"[LogiSense] Fusion complete — mean pre-risk {risk_scores.mean():.3f}")
