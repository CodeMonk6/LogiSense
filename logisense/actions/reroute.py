...
class RerouteExecutor:
        ...
        for src in preds:
            ...
            try:
                ...
                paths = list(nx.all_simple_paths(
                    self.network.graph, src, target_node, cutoff=3
                ))
            except Exception:
                paths = []
            ...
            for path in paths[:max_alternates]:
                if len(path) < 2:
                    continue
                alt_lt   = sum(self.network.graph[path[i]][path[i+1]].get("lead_time", 10)
                               for i in range(len(path) - 1))
                alt_cost = sum(self.network.graph[path[i]][path[i+1]].get("cost", 1)
                               for i in range(len(path) - 1))
                alt_risk = max(
                ...
                score = (self.alpha * (alt_lt - current_lt) / max(current_lt, 1)
                         + self.beta  * (alt_cost - current_cost) / max(current_cost, 1)
                         + self.gamma * alt_risk)

                plans.append((score, ReroutePlan(
    ...
    def execute(self, plan: ReroutePlan, dry_run: bool = True) -> Dict:
        ...
        """
        if dry_run:
            logger.info("[Reroute DRY-RUN] %s → %s (+$%.0f, +%.1fd)",
                        plan.original_lane, plan.alternate_lane,
                        plan.extra_cost, plan.extra_days)
        else:
            logger.info("[Reroute EXECUTE] %s → %s", plan.original_lane, plan.alternate_lane)
