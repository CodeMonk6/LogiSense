...
_SAMPLE_ALTERNATES: Dict[str, List[SupplierProfile]] = {
    "node_000": [
        SupplierProfile("alt_sup_001", "Alternate Supplier A", 12.0, 1.05, 0.92, 0.10, 10_000, ["ITEM_A", "ITEM_B"]),
        SupplierProfile("alt_sup_002", "Alternate Supplier B", 18.0, 0.98, 0.88, 0.15, 8_000,  ["ITEM_A"]),
    ],
    "node_001": [
        SupplierProfile("alt_sup_003", "Alternate Supplier C", 10.0, 1.12, 0.95, 0.08, 12_000, ["ITEM_C"]),
    ],
}
...
class ProcureExecutor:
    ...
    def evaluate(
        ...
        for alt in alternates:
            ...
            score = (0.5 * alt.lead_time_days / 30.0
                     + 0.3 * (alt.unit_cost - 1.0)
                     + 0.2 * alt.risk_score)

            candidates.append((score, ContingentPO(
    ...
    def execute(self, po: ContingentPO, dry_run: bool = True) -> Dict:
        ...
        if dry_run:
            logger.info("[Procure DRY-RUN] PO %s: %s units from %s — $%.0f",
                        po.po_id, po.quantity, po.alt_supplier, po.total_cost)
        else:
            po.status = "CONFIRMED"
